...
qualidade. Assim, força todo o tráfego, dirigido ao destino alvo do ataque,  a circular através
dele. Por exemplo, no protocolo AODV\cite{Perkins1999}, por ser \textit{on-demand} permite que, na
fase de descoberta de uma rota, qualquer nó, que possua um caminho (suficiente recente), responda a
uma mensagem de RREQ. Com isto, este algoritmo de encaminhamento pode ficar sujeito a um ataque
de \textit{blackhole}, pois um nó malicioso intermédio, pode responder com um caminho melhor,
apesar de não ter sequer caminho para o destino, originando um ``buraco negro'', interrompendo o
processo de comunicação\cite{Survey_wsn_Sec_issues,Attaks_defenses_sec_in_wsn}.
\end{description}

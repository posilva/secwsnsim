...
\cite{ietf_802154,zigbee_802154} e Zigbee \cite{zigbee_802154})). A possibilidade de construção
destes dispositivos (que se designam mais simplesmente por nós sensores) fez surgir, nos últimos
anos, um novo campo da investigação conhecido por redes de sensores sem fios (RSSF).
...
humana, por exemplo, para o carregamento ou substituição de baterias. Em certos ambientes de
instalação pode não ser possível, ou viável, realizar operações que exijam intervenção ou supervisão
...
carregamento expedito de código desenvolvido em estações de desenvolvimento. Os
sensores dotados de ligações Ethernet, RS-232 ou USB podem ainda funcionar como nós de tipo gateway,
...
\textit{software}, \figurename{ \ref{stack_serv}}). Diferentes tipologias de ataques 
...
relevante quanto a análise de segurança pode envolver a avaliação de diferentes modelos e hipóteses
...
forma sistemática e comparativa, na abordagem de diferentes protocolos de encaminhamento seguro que
...
Para cada protocolo interessa medir o impacte que diferentes tipologias de ataques podem ter
nomeadamente, em cenários de grande escala, o que exige a utilização de sistemas de simulação
\cite{ns2,jsim,SENSE,shox,jprowler,freemote} de RSSF que permitam simular diferentes hipóteses de
ataque e antecipar o seu impacte. Este impacte deve ser analisado não apenas no que refere ao
comportamento dos protocolos mas, complementarmente, no que refere à forma como afectam a própria
rede nomeadamente, em termos de consumo de energia, fiabilidade e latência. Um sistema que suprima
...
No âmbito do trabalho que se pretende desenvolver na elaboração da dissertação, irá ser concebido e
desenvolvido um sistema de simulação inovador. Este sistema deverá permitir o estudo sistemático de
protocolos de encaminhamento, desenhados para serem seguros, e deverá possuir, em particular, as
seguintes funcionalidades: 
\begin{itemize}
\item Interface de visualização e configuração da rede com informações dos parâmetros de simulação e
informação de cada nó (por exemplo, o seu estado energético); 
...
\item Modelo parametrizável de geração de topologias, podendo estas ser definidas como: distribuição
...
fiabilidade, correcção do protocolo e correcção dos eventos, disponibilizando-os de forma gráfica. 
\end{itemize}
O ambiente de simulação em causa permitirá estudar, sistemática e comparativamente, diferentes
protocolos de encaminhamento seguro em RSSF. Contribuirá, ainda, com o estabelecimento de uma base
de comparação, pelo desenvolvimento de dois protocolos, para futuras avaliações de outros protocolos
emergentes. Esta avaliação será referente ao impacte que os mecanismos de segurança têm sobre
propriedades tão importantes como: o consumo de energia, a fiabilidade da rede, a latência das
comunicações, correcção dos dados e a correcção do protocolo. Estas propriedades são,
normalmente,extrapoladas da implementação experimental em pequena escala ou da adopção de modelos
matemáticos, os quais, devido a variáveis externas, próprias do ambiente de operação, se podem 

...
uma rede para os milhares de dispositivos pode representar valores bastante elevados. No campo da
investigação estão a ser
...
limitado, o uso real de sensores apresenta-se como uma forma pouco eficiente para o desenvolvimento
de novas tecnologias ou melhoramento das existentes.
...
todas as tecnologias associadas, antes destas se
implementarem\cite{park_simulating_2001,simulators}. Alguns dos
ambientes conhecidos foram adaptados de outros já existentes, como é o caso do NS2\cite{ns2} ou
J-Sim\cite{jsim}. Uma propriedade importante destes ambientes é a
capacidade de rápidamente repetir experiências nas mesmas condições recorrendo a parâmetros 
configuráveis (ex. nº de nós, modelo de rádio)

Nesta secção apresentam-se diversos ambientes de simulação, mais comuns, e que permitam simular
sistema de RSSF. Foram seleccionados, em premeiro lugar, seguindo os critérios relacionados com 
...
\subsubsection{Relacionados com Engenharia de
Software}
...
orientada a objectos ( permitindo reutilização). Assim, foram seleccionadas apenas ambientes
...
\end{description}
\subsubsection{Relacionados com as RSSF}
\begin{description}\descvspace
...
distríbuidos por uma vasta àrea. Assim, é importante que o ambiente de simulação possa suportar a
simulação com milhares de nós, uma vez que o factor escala é um das propriedades que se querem ver
analisadas no trabalho a desenvolver;
\item[Modelo de Colisões/Comunicação Rádio] Este modelo é fundamental que se encontre modelado no
sistema de simulação. Por ser um componente base das RSSF e possuir características 
relacionadas com a simulação da camada de acesso ao meio e ligação de dados. Estas possuem
espeficidades inerentes aos protocolos implementados (ex: B-MAC, S-MAC)\cite{BMAC,SMAC};
...
\item[Interface de Visualização] É importante que o ambiente de
simulação possua uma interface de visualização, o que permite uma mais fácil percepção dos
comportamentos dos protocolos (ex: topologia, cobertura), bem como o controlo da simulação e
...
topologias estáticas e outro de  para topologias móveis.
\subsection{J-Sim}
...
aplicações.
\subsection{Freemote}
Fremote é uma ferramenta de emulação \cite{freemote} \footnote{Técnica onde as propriedades de
...
de comunicação IEEE802.15.4\cite{ietf_802154} (ex:MICAz\cite{micaz}).
...
com nós reais baseados em Java e na norma IEEE802.15.4\cite{ietf_802154}. Os principais pontos
...
Java\cite{shox}. A ideia principal desta ferramenta é a de proporcionar, uma

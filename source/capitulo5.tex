...
de mensagens é a garantia de não repetição encaminhamento de mensagens. 
...
\textit{Hello}. No diagrama de classes da figura seguinte apresenta-se esta
organização.
\input{pic_FloodingRoutingcd}
Assim, no envio de mensagens entre quaisquer dois sensores A e B na rede o
protocolo executa o seguinte algoritmo:

\begin{algorithm}[H]
\SetLine
\Entrada{Lista de mensagens enviadas, mensagem recebida}
\uSe{listaMensagensEnviadas.existe(m)}{
  descarta( m);
}
\Senao{
  listaMensagensEnviadas.insere(m);\\
  encaminhaMensagem(m);
}
\caption{Protocolo de \textit{flooding} sem repetição}
\label{alg:flooding}
\end{algorithm}
...
executam também um conjunto de operações das quais se distinguem as seguintes:
...
\subsubsection{Análise critica}
Para a implementação do protocolo INSENS, procurou-se respeitar ao máximo a
implementação. Alguns pormenores foram relaxados tendo em conta a sua
implementação numa base de simulação. Estes pormenores em nada influem na
execução do protocolo com especificada
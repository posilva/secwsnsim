...
O desenho do mecanismo de injecção de ataques ao nível do encaminhamento está
directamente associado ao RoutingLayer nas duas acções principais deste: o
envio e a recepção de mensagens. Assim sendo, um ataque cuja acção seja no
momento de receber a mensagem (por exemplo um \textit{Rushing Attack}) ou no
envio da mensagem (\textit{Sybil Attack}) podem ser modelados através destes
dois interceptores. No entanto, algures na operação da lógica de um protocolo
podemos querer induzir um comportamento específico, para isso um ataque pode
ainda ser executado a pedido com a passagem de um comando. Na figura seguinte
apresenta-se a taxonomia de uma classe que implementa um ataque.

CLASSE AQUI COM COMENTÁRIOS
